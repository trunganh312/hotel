(() => {
  ...
  var e = {
    ...
    d: (i, t) => {
      for (var r in t)
        e.o(t, r) && !e.o(i, r) && Object.defineProperty(i, r, { enumerable: !0, get: t[r] });
    },
  ...
  class h {
    ...
    checkOptions() {
      if (this.options.mode !== h.mode.url) throw `暂不支持的模式 ${this.options.mode}`;
      if ("text" !== this.$originEle.attr("type") && this.$originEle.attr("type"))
    ...
    initEle() {
      if (
        ...
        (this.$uploaderContainer = t()('<div class="jquery-uploader-preview-container"></div>')),
    ...
    onFileUploadSuccess(e, i) {
      ...
      try {
        if (((t = this.options.ajaxConfig.responseConverter(e, i)), !t.url))
          return console.error("No URL"), void this.onFileUploadError(e, "No URL");
      } catch (i) {
    ...
    handleFileDelete(e) {
      let $card = t()(e.target).closest(".jquery-uploader-card");
      let fileId = $card[0].id;

      // Tìm file trong danh sách
      let fileIndex = this.files.findIndex((file) => file.id === fileId);
      if (fileIndex !== -1) {
        let file = this.files[fileIndex];
        let nameExtension;
        let name;

        // Check xem ảnh xóa từ db hay anh xóa vừa gửi lên
        if (file.type) {
          nameExtension = file.name.split(".")[1];
          name = `${file.id}.${nameExtension}`;
        } else {
        ...
        $.ajax({
          url: "delete_handler.php", // Đường dẫn đến tập lệnh PHP xử lý xóa
          type: "POST",
          data: { name: name }, // Gửi tên tệp
          success: (response) => {
            ...
            try {
              ...
              if (result.success) {
                ...
                this.refreshValue();
                this.refreshPreviewFileList();
                this.$originEle.trigger("file-remove", file);
              } else {
                alert("Xóa ảnh thất bại: " + result.message);
              }
            } catch (e) {
              console.error("Lỗi xử lý phản hồi từ máy chủ:", e);
