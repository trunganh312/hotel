...
/**
 * Class HTMLCleanup
 * Modified by SenEnter
 */
class HTMLCleanup{
	
    //Các tag cho phép
	private $valid_elements    =   ["a", "b", "blockquote", "br", "center", "del", "div", "em", "font", "h2", "h3", "h4", "i", "img", "ins", "li", "hr", "ol",
	...
	private $extended_valid_elements = array("a"              => array("href", "name", "rel", "style", "target", "title"),
										 "b"              => array("style"),
	...
	//Các attribute không được phép dùng
	private $invalid_styles    =   array("behavior", "background-image", "background", "list-style-image", "expression", "/*", "*/");
	
	//Các attribute được phép dùng để override invalid_styles (ví dụ background bị xóa thì vẫn phải cho background-color)
	private $override_styles   =   array("background" => array("background-color"),);
	
	//Các giao thức được dùng
	private $web_protocol  =   array("http://", "https://", "ftp://", "mailto:", "tel:");
	private $tag_single    =   array("img", "br", "hr", "meta", "input");
	private $char_remove   =   array();
	public $input_html;
	public $output_html;
	private $DOMDoc;
	//Lưu lại log
	private $log_string        = "";
	public $max_len_image_name = 80;	
	...
	function __construct($input_html){
        ...
        //Remove JS
        $input_html  =   str_replace('&lt;script&gt;', '', $input_html);
        $input_html  =   str_replace('&lt;/script&gt;', '', $input_html);
        $input_html  =   str_replace('script', '', $input_html);
	...
	function clean(){
		...
		//Sau khi đã trải qua công đoạn làm sạch gán output = input
		$this->output_html = $this->input_html;
		
		//Cleanup HTML Comment
		$this->output_html = preg_replace('/&lt;!--(.|\s)*?--&gt;/', '&nbsp;', $this->output_html);
        
        //Xóa các CSS có !important để ko bị đè lên các CSS của template
        $this->output_html  =   str_replace('!important', '', $this->output_html);
					...
					//Đối với attribute có trong định nghĩa thì check style, src, href chống nhét Javascript
						...
						switch ($attribute_name){
							...
							case "href":
								...
								// Cmt lại để k bị add vào nội dung
